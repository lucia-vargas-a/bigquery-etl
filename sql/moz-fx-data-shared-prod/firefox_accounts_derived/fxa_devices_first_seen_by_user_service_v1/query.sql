WITH fxa_events AS (
  SELECT
    ...
    ua_browser,
  FROM
    `moz-fx-data-shared-prod.firefox_accounts.fxa_content_auth_oauth_events`  -- # TODO: there seems to be fxa_content_auth_events, maybe we should use that instead since oauth has been deprecated for a long time now?
  WHERE
    DATE(`timestamp`)
...
WHERE
  user_id IS NOT NULL
QUALIFY
  ROW_NUMBER() OVER (PARTITION BY user_id, service, device_id ORDER BY first_seen_date ASC) = 1  -- this could be partitioned by first_flow_id to handle multiple device_ids being attached to a single flow_id
